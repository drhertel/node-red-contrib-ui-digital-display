	...
	RED.nodes.registerType( 'ui_digital_display', function( config ) {
		...
		if( ui && RED.nodes.getNode( config.group ) ) {
			this.on( 'close', ui.addWidget( {
				node: this,
				format: `
					<div id="ui_digital_display_{{ $id }}" style="display: flex; height: 100%; justify-content: flex-end"
						 ng-init="digits = ${ +config.digits || 0 }; decimals = ${ +config.decimals || 0}; padding = '${config.padding || ' '}'"> 
				...
				initController: $scope => {
					$( () => $scope.msg = !!$scope.msg ? { ...$scope.msg } : { payload: 0 } );
					...
					$scope.$watch( 'msg', msg => {
						...
						if( value.length > $scope.digits ) {
							...
							value = value.padStart( $scope.digits, $scope.padding.toString());
						}
